...
var add2Tag = function (_string, _tag2add, _name, _type2add){

	var greatThanPosition = getCharPosition(_string, ">")
	  , names = ""
	  , charPosition
	  , position
	  , stop
	...
	function extractNumbers(){
		for (var i = 0; i < _tag2add.length; i++) {
			if(isNumber(_tag2add[ i ] ) || 
			   isDigit(_tag2add[ i ])){
				stop = parseInt(_tag2add[ i ]);
				_tag2add = remove(_tag2add, _tag2add[ i ]) ;
			}
		} 
	}

	// with the next statements, i'm looking for numbers in the _tag2add variable,
	// that number will be the choosen tag.
	if(isAlphanumeric(_tag2add)){extractNumbers();}
	if(isNullOrUndefined(stop)){stop = 1;}


	// if there a !DOCTYPE html tag(<!DOCTYPE html>), in the _string variable,
	// and you want to add some styles to the html tag (<html>),
	// the next statement, will allow you to do it.
	...
	// looking for the correct position, to add the class or id or style.
	for (var i = 1; i <= greatThanPosition.length; i++) {
		if(_tag2add[_tag2add.length - 1] == _string[greatThanPosition[i - 1] - 1] 
			||  _string[greatThanPosition[i - 1]] == ">"){
			charPosition = getCharPosition(_string, _tag2add[_tag2add.length - 1]);
			if(charPosition.length >= 2){
				if(stop > charPosition.length){return _string;}
				else{
					for (var j = 1; j <= charPosition.length; j++) {
						if (stop == j) {
							if(stop <= (charPosition.length / 2) 
							   && _string[greatThanPosition[j - 1] -2] != '/'){
								position = charPosition[j - 1] + 1;
								break; // ending the second for loop
							}else{
								return _string;
							}
						}
			...
			}else{position = charPosition[0] + 1;}
			// ending the first for loop.
			break; 
		}
	}

	// looking for class' attribute in the choosen tag.
	// looking for id's attribute in the choosen tag.
	// looking for style's attribute in the choosen tag.
	for (var i = 0; 1; i++){
		if(_string[position + i] == ">" ){
			_ = _string.slice(position + 1, (position - 1) + (i + 1));
			break;
		}
	}
	
	// looking for classname, idname, styles in the 
	// choosen tag...
	for (var i = 0; i < _.length; i++) {
		if(_string[position + i] + _string[position + i + 1] 
			==  _type2add[_type2add.length - 1 ] + "="){
			for (var j = 0; j < _.length; j++) {
				if(_string[position + i + 3 + j] == "'"){
					__ = _string.slice(position + i + 2, position + i + 3 + j);
					break;
				}
			}
			__ = remove(__, "'");
	...
	// checking, if there another similar classname, idname, styles, 
	// in the choosen tag. if there one or more than one repetition,
	// it not be added. 
	if(isArray(_name)){
		if(_name.length >= 2){
			for (var i = 0; i < _name.length; i++) {
				if(!isSubString(__, _name[i])){
					if(i == _name.length - 1){names += _name[i];}
					else{names += _name[i] + " ";}
				}
			}
		}else{
			if(!isSubString(__, _name[0])){
				names = _name[0];
			}
		}
	}else{
		if(!isSubString(__, _name)){
			names = _name;
		}
	}

	// if class attribute is in the choosen tag, the class name(s), will be added. 
	// if id attribute is in the choosen tag, the id name(s), will be added.
	// if style attribute is in the choosen tag, the style(s), will be added.
	if(isSubString(_, _type2add)){
		for (var i = 0; i < _.length; i++) {
			if(_.slice(i, _type2add.length + i) == _type2add){
				position += _type2add.length + __.length + 3;
				break;
			}
			position += 1;
		}
		// if the -names- variable, is empty, nothing will be added.
		names = names != "" ? " " + names: "";
	}else{
		names = " " + _type2add + "='" + names + "'";
		position = position;
	}
...
var add2TagClassName = function(_string, _tag2addclassname, _classname){
	...
	**/
	...
	if(isNullOrUndefined(_string)){
		...
		if(isNullOrUndefined(_classname) || isNullOrUndefined(_tag2addclassname)){
			...
			return add2Tag(_string, _tag2addclassname, _classname, "class");
		}
	}
};
...
var add2TagIdName = function(_string, _tag2addidname, _idname){
	...
	**/
	...
	if(isNullOrUndefined(_string)){
		...
		if(isNullOrUndefined(_idname) || isNullOrUndefined(_tag2addidname)){
			...
			return add2Tag(_string, _tag2addidname, _idname, "id");
		}
	}
};
...
var add2TagStyle = function(_string, _tag2addstyles, _styles){
	...
	**/
	...
	if (isNullOrUndefined(_string)){
		...
		return add2Tag(_string, _tag2addstyles, styles, "style");
	}
};
exports.add2TagStyle = add2TagStyle;


var add2TagAttr = function(_string, _tag2addattr, attrname , attrs){
	...
	**/
	...
	if(isNullOrUndefined(_string)){
		...
		if(isNullOrUndefined(_tag2addattr) || isNullOrUndefined(attrname) 
			&& attrname != "class" && attrname != "id" && attrname != "style"){
			return _string;
		}else{
			...
			if (isEmpty(attrs) || attrs == "") {
				var _result = add2Tag(_string, _tag2addattr, attrs, attrname);
				...
				return add2Tag(_string, _tag2addattr, attrs, attrname);	
			}
		}
	}
}
exports.add2TagAttr = add2TagAttr;