...
exports.toCharArray = function(_string) {
...
exports.insert = function(_string, _value, _position) {
	...
	**/
	...
	if(isNullOrUndefined(_position)){
		_position = this.getSize(_string) - 1;
	}

	if(_position == this.getSize(_string)){
		return _string + _value;
	}else if (i > this.getSize(_string)){
		return _string;
	}

	var _array = this.toCharArray(_string);
	var _nString = "";

	for (var i = 0; i < this.getSize(_string) ;i++) {
		if(i == _position){
			_array[i] = _value;
			for (var j = 1; j <= this.getSize(this.lSlice(_string,i)); j++) {
				_array[i+j] = this.lSlice(_string, i)[j-1];
			}
			break;
		}
	}
...
exports.delete = function(_string, _value) {
	...
	**/
	...
	return this.lReplace(_string, _value, "");
}


exports.center = function(_string, _times, _val) {
	...
	**/
	...
	return this.multiply(_val, _times) + _string + this.multiply(_val, _times);
};	


exports.lJust = function(_string, _times, _val) {
	...
	**/
	...
	return this.multiply(_val, _times) + _string;
};	


exports.rJust = function(_string, _times, _val) {
	...
	**/
	...
	return _string + this.multiply(_val, _times);
};	
...
exports.dummyAddTag = function(_string, _tagName) {
	...
	**/
	...
	if(_tagName == "img"){
		_ = "/";
		return this.insert(this.lConCat(_string, _startTag), _, 4);
	}else if(_tagName == "meta"){
		return this.lConCat(_string, _startTag + _); 
	}else if(_tagName == "DOCTYPE" || this.isSubString(_tagName, "DOCTYPE")){
		_ = "!";
		if (this.isSubString(_tagName, "html" )){
			return this.insert(this.lConCat(_string, _startTag ), _, 1);
		}else{
			return this.insert(this.lConCat(_string, this.insert(_startTag, " html") ), _, 1);
		}
	}
...
exports.removeTag = function(_string) {
...
exports.smartAddTag = function(_string, _tag_s) {
	...
	**/
	...
	if(!isArray(_tag_s)){
		return this.dummyAddTag(_string, _tag_s);
	}else{

		for (var i = this.getSize(_tag_s) -1; i >= 0 ; i--) {
			_string = this.dummyAddTag(_string, _tag_s[i]);
		}
