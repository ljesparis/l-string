...
var lSlice = function(_string, _start, _end) {
...
var rSlice = function(_string, _start, _end) {
	...
	**/
	...
	if (isNullOrUndefined(_end)){
		_end = getSize(_string);
	}

	var _stringLength = getSize(_string);
...
var dummyCount = function(_string, _subString, _startCount, _endCount) {
	...
	**/
	...
	if (isNullOrUndefined(_endCount)){
		_endCount = getSize(_string);
	}

	var _subStringLength = getSize(_subString);
	...
	for (var i = _startCount; i < _endCount; i++) {
		...
		_slice = lSlice(_string, _startSilce, _endSlice);

		if (_subString == _slice){
			count ++;
		}

		var _arraySlice = getCharPosition( _string, _slice[ getSize(_slice) - 1] );
		var _sizeArray = getSize(_arraySlice);
...
var capitalize = function(_string) {
	...
	**/
	...
	return lReplace(getLower(_string), 
						 getLower(_string)[0],
						 getLower(_string)[0].toUpperCase());
};
...
var getCharPosition = function(_string, _char) {
	...
	**/
	...
	for (var j = 0; j < getSize(_char); j++){
		for (var i = 0; i < getSize(_string); i++){
...
var reverse = function(_string) {
	...
	**/
	...
	for (var i = getSize(_string); i > 0; i--){
...
var title = function(_string) {
	...
	**/
	...
	var _newString = "";
	var _stringLower = getLower(_string);
...
var lSplit = function (_string, _subS, maxsplit) {
	...
	**/
	...
	if (isSubString(_string, _subS)){
		...
		var _nString = "";
		var _subStringLength = getSize(_subS);
		var _slice;
		var _startSlice = 0;
		...
		for (var i = 0; i < getSize(_string); i++){
			...
			_slice = lSlice(_string, _startSlice, _endSlice);

			if (_subS != _slice){
				_nString += lSlice(_slice, 0,1);
			}else{
				if (_nString){
...
var rSplit = function (_string, _subS, maxsplit) {
	...
	**/
	...
	if (isSubString(_string, _subS)) {
		...
		for (var i = getSize(_string) - 1 ; i >= 0; i--){
	...
	for (var i = getSize(_string_array_copy) - 1; i >= 0; i--){
...
var isSubString = function(_string, _subString) {
	...
	**/
	...
	var _startSilce = 0;
	var _endSlice = getSize(_subString);
	var _slice;
...
var startsWith = function(_string, _prefix) {
	...
	**/
	...
	var _prefix_len = getSize(_prefix);
	var cont = 0;
...
var endsWith = function(_string, _prefix) {
	...
	**/
	...
	var cont = 0;
	var _prefixLength = getSize(_prefix);
	var i = getSize(_string);
	var j = _prefixLength;
...
var lReplace = function(_string, _old, _new, maxreplace) {
	...
	**/
	...
	if (isSubString(_string, _old) && _old != _new){
		...
		var count = 0;
		var _oldLength = getSize(_old);
		var _slice;
		var _startSlice = 0;
		var _endSlice = _oldLength;

		for (var j = 0; j < getSize(_string); j++) {
...
var join = function (_string, _array) {
	...
	**/
	...
	for (var i = 0; i < getSize(_array); i++) {
...
var insert = function(_string, _value, _position) {
	...
	**/
	...
	if(isNullOrUndefined(_position)){
		_position = getSize(_string) - 1;
	}

	if(_position == getSize(_string)){
		return _string + _value;
	}else if (i > getSize(_string)){
		return _string;
	}
	...
	for (var i = 0; i < getSize(_string) ;i++) {
		if(i == _position){
			_array[i] = _value;
			for (var j = 1; j <= getSize(lSlice(_string,i)); j++) {
				_array[i+j] = lSlice(_string, i)[j-1];
			}
			break;
		}
	}
...
var smartAddTag = function(_string, _tag_s) {
	...
	**/
	...
	if(!isArray(_tag_s)){
		...
		for (var i = getSize(_tag_s) -1; i >= 0 ; i--) {
...
var add2Tag = function (_string, _name, _tag2add, _type2add){
	...
	if(isAlphanumeric(_tag2add)){
		...
		if( isNumber(_tag2add[0]) || isDigit(_tag2add[0]) ){
			
			stop = _tag2add[0];
			_tag2add = remove(_tag2add, _tag2add[0]);

		}else if( isNumber(_tag2add[ getSize(_tag2add) -1 ] ) || 
				isDigit(_tag2add[getSize(_tag2add) -1]) ){

			stop = _tag2add[getSize(_tag2add) -1];
			_tag2add = remove(_tag2add, 
					    			_tag2add[getSize(_tag2add) -1]);
	...
	if(getSize(position) >= 3){
		if(isSubString(_string, _tag2add)){

			char_ = _tag2add[getSize(_tag2add) - 1];
			charPosition = getCharPosition(_string, char_);

			if( (getSize(charPosition) / 2) < stop && 
				(getSize(charPosition) / 2) > 1 ){
				return _string;
			}

			if(getSize(charPosition) >= 2){

				for(var i = 0; i < getSize(charPosition); i++){
		...
		if(isSubString(_string, _tag2add) && 
		   ( (getSize(position) == 1 && stop == 1) || 
			 (stop == 1 && getSize(position) == 2) )){
