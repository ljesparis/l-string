...
var multiply = function(_string, _times) {
	 ...
	 *
	 *	@Argument: _string ==> simple string.
	 *	@Argument: _times  ==> number of times that you want to repeat the _string variable.
	 *
	 **/
...
var rSlice = function(_string, _start, _end) {
	 ...
	 *
	 *	@Argument: _string ==> simple string.
	 *	@Argument: _start  ==> where the slice begins.
	 *	@Argument: _end    ==> where slice ends .
	 *
	 **/
...
var dummyCount = function(_string, _subString, _startCount, _endCount) {
	 ...
	 *
	 *	@Argument: _string     ==> simple string.
	 *	@Argument: _subString  ==> substring that you want to count.
	 *	@Argument: _startCount ==> where count begins (if undefined, will count all substrings).
	 *	@Argument: _endCount   ==> where count ends.
	 *
	 **/
...
var getCharPosition = function(_string, _char) {
	 ...
	 *
	 *	@Argument: _string ==> simple string.
	 *	@Argument: _char ==> simple character, that this function will it find.
	 *
	 **/
...
var isSubString = function(_string, _subString) {
	 ...
	 *
	 *	@Argument: _string     ==> simple string.
	 *	@Argument: _subString  ==> simple string.
	 *
	 **/
...
var rConCat = function(_string, _newValue) {
	 ...
	 *
	 *	@Argument: _string    ==> simple string.
	 *	@Argument: _newValue  ==> value that you want to concatenate to the string.
	 *
	 **/
...
var lReplace = function(_string, _old, _new, maxreplace) {
	 ...
	 *
	 *	@Argument: _string     ==> simple string.
	 *	@Argument: _old        ==> old value that you want to replace in the string.
	 *	@Argument: _new        ==> new value that you want to set in the string.
	 *	@Argument: maxreplace  ==> number of times you want to replace the _old value.
	 *
	 **/
	if (isNullOrUndefined(_string)) {
		return "";
...
var join = function(_string, _array) {
	 ...
	 *
	 *	@Argument: _string  ==> simple string.
	 *	@Argument: _array   ==> matrix you want to unite with "-" .
	 *
	 **/
...
var insert = function(_string, _value, _position) {
	 ...
	 *
	 *	@Argument: _string   ==> simple string.
	 *	@Argument: _value    ==> value that you want to insert.
	 *	@Argument: _position ==> position where you want to insert the value.
	 *
	 **/
...
var remove = function(_string, _value) {
	 ...
	 *
	 *	@Argument: _string ==> simple string.
	 *	@Argument: _value  ==> value that you want to remove.
	 *
	 **/
...
var dummyAddTag = function(_string, _tagName) {
	 ...
	 *
	 *	@Argument: _string    ==> simple string.
	 *	@Argument: _tagName   ==> tag that you want to add.
	 *
	 **/
...
var smartAddTag = function(_string, _tag_s) {
	 ...
	 *
	 *	@Argument: _string  ==> simple string.
	 *	@Argument: _tag_s   ==> n-tags that you want to add.
	 *
	 **/
...
var extractNumbers = function(_tag) {
	var tagNumber;
	if (_tag[0].isDigit() || _tag[0].isNumber()) {
		tagNumber = parseInt(_tag[0]);
		_tag = _tag.lReplace(_tag[0], "", 1);
	} else if (_tag[_tag.length - 1].isDigit() || _tag[_tag.length - 1].isNumber()) {
		if (!_tag[_tag.length - 2].isAlpha() ||
			_tag[_tag.length - 2].isAlpha()) {
			tagNumber = parseInt(_tag[_tag.length - 1]);
			_tag = _tag.rReplace(_tag[_tag.length - 1], "", 1);
		} else {
			_tag = _tag;
			tagNumber = 1;
		}
...
var add2Tag = function(string, _tag, attr, attrValue) {
	var _ = extractNumbers(_tag),
		tagPosition = _.tagNumber, // tag tagNumber.
		charPosition = string.getCharPosition("<"),
		tag = "",
		_attrValue = "",
		m = 0, // counter
		n = 0; // counter
	...
	for (var i = 0; i < string.length; i++) {
		if (string[i] == ">") {
			tag = string.slice(charPosition[m], i + 1);
			if (!tag.isSubString("</") &&
				!tag.isSubString("DOCTYPE") &&
				tag.isSubString("<" + _tag)) {
				n++;
				if (tagPosition == n) {
					if (!tag.isSubString(attr + "=")) {
						return string.insert(" " + attr + "='" + _attrValue + "'", i);
					} else {
						var j = i;
						for (var j = i; true; --j) {
							if ((attr + "=") == string.slice(j - (attr + "=").length, j)) {
								return string.insert(_attrValue + " ", j + 1);
							}
						}
					}
				}
...
exports.add2Tag = add2Tag;


var add2TagClassName = function(_string, _tag, _classvalue) {
	 ...
	 *
	 *	@Argument: _string      ==> simple string.
	 *	@Argument: _tag         ==> Tag where you want to add some classname.
	 *	@Argument: _classvalue  ==> value of the class attribute...
	 *
	 **/
	...
	if (isNullOrUndefined(_string)) {
		...
		if (isNullOrUndefined(_classvalue) || isNullOrUndefined(_tag)) {
			return _string;
		} else {
...
var add2TagIdName = function(_string, _tag, _idvalue) {
	 ...
	 *
	 *	@Argument: _string  ==> simple string.
	 *	@Argument: _tag     ==> Tag where you want to add some idname.
	 *	@Argument: _idvalue ==> value  of the id attribute ...
	 *
	 **/
	...
	if (isNullOrUndefined(_string)) {
		...
		if (isNullOrUndefined(_idvalue) || isNullOrUndefined(_tag)) {
			return _string;
		} else {
...
var add2TagStyle = function(_string, _tag, _styles) {
	 ...
	 *
	 *	@Argument: _string  ==> simple string.
	 *	@Argument: _tag     ==> Tag where you want to add some styles.
	 *	@Argument: _styles  ==> Styles that you want to add.
	 *
	 **/
...
var add2TagAttr = function(_string, _tag, attrname, attrs) {
	 ...
	 *
	 *	@Argument: _string   ==> simple string.
	 *	@Argument: _tag      ==> Tag where you want to add some attributes.
	 *	@Argument: attrname  ==> attribute name that you want to add.
	 *	@Argument: attrs     ==> attributes that you want to add.
	 *
	 *	Example:
